...
const totalAmount = document.getElementById("calculated-buy-total");
const cryptoChoice = document.getElementById("crypto-choice");
const buyDollars = document.getElementById("buy-dollars");
buyAmount.addEventListener("keyup", function(){
	$.ajax(liveCrypto).done(function (response) {
		totalAmount.textContent = toTwoDecimal(buyAmount.value * response[cryptoChoice.value].usd);
		buyDollars.value = toTwoDecimal(buyAmount.value * response[cryptoChoice.value].usd);
	});
});

cryptoChoice.addEventListener("change", function(){
	$.ajax(liveCrypto).done(function (response) {
		buyAmount.value = "";
...
buyDollars.addEventListener("keyup", function(){
	$.ajax(liveCrypto).done(function (response) {
		buyAmount.value = buyDollars.value / response[cryptoChoice.value].usd;
		totalAmount.textContent = buyDollars.value;
	});
})


const sell = document.getElementById("sell-amount");
const sellTotalAmount = document.getElementById("sell-total");
const cryptoChoiceSell = document.getElementById("crypto-choice-sell");
const sellDollars = document.getElementById("sell-dollars");
sell.addEventListener("keyup", function(){
	$.ajax(liveCrypto).done(function (response) {
		sellTotalAmount.textContent = toTwoDecimal(sell.value * response[cryptoChoiceSell.value].usd);
		sellDollars.value = toTwoDecimal(sell.value * response[cryptoChoiceSell.value].usd);
	});
});

cryptoChoiceSell.addEventListener("change", function(){
	$.ajax(liveCrypto).done(function (response) {
		sell.value = "";
		sellTotalAmount.textContent = "";
		sellDollars.value = "";
	});
});


sellDollars.addEventListener("keyup", function(){
	$.ajax(liveCrypto).done(function (response) {
		sell.value = sellDollars.value / response[cryptoChoiceSell.value].usd;
		sellTotalAmount.textContent = sellDollars.value;
	});
})
...
function fillTable(){
	$.ajax(liveCrypto).done(function (response) {
		...
		for(let i = 1; i < tbody.children.length; i++){
			let coin = tbody.children[i].children[0].textContent.replace(/ /g,'').slice(0,-1);
			let priceNow = parseFloat(response[coin].usd);
			let units = parseFloat(tbody.children[i].children[1].textContent);
			tbody.children[i].children[2].textContent = priceNow;
			tbody.children[i].children[3].textContent = toTwoDecimal(priceNow * units);
			total = total + (priceNow*units);
		}
		total += parseFloat(tbody.children[0].children[1].textContent);
		document.getElementById("total-assets").textContent = toTwoDecimal(total);
	});
}